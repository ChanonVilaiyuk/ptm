...
# 			return sgStatus
	...
	def getTaskInfo(self) : 
		...
		for each in taskInfo : 
			...
			if entity : 
				...
				if entityType == 'Asset' : 
					...
					entities.update({displayName: {'status': status}})
		...
		for each in infoDict : 
			for eachTask in infoDict[each] : 
				taskName2 = eachTask
				content = infoDict[each][taskName2]
				
				if not taskName2 in allTasks : 
					allTasks.update({taskName2: content})
					
				else : 
					print '- %s' % infoDict
					allTasks[taskName2].update(content)
					print '- %s' % infoDict

		# info.update({'all': allTasks})
